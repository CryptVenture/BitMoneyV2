...
void PrivacyDialog::sendzsnd()
{
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BitMoney </b>will be added to the standard transaction fees!<br />";
    ...
    if (!ZBitMoneyControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBitMoneyControlDialog::GetSelectedMints();
    }
    ...
    // Clear zBitMoney selector in case it was used
    ZBitMoneyControlDialog::listSelectedMints.clear();
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " BitMoney, ";
