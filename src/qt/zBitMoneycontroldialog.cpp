...
#include "zBitMoneycontroldialog.h"
#include "ui_zBitMoneycontroldialog.h"
...
std::list<std::string> ZBitMoneyControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBitMoneyControlDialog::listMints;

ZBitMoneyControlDialog::ZBitMoneyControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBitMoneyControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBitMoneyControlDialog::~ZBitMoneyControlDialog()
{
    delete ui;
}

void ZBitMoneyControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBitMoneyControlDialog::updateList()
{
...
void ZBitMoneyControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBitMoneyControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBitMoney_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBitMoneyControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBitMoneyControlDialog::GetSelectedMints()
{
...
void ZBitMoneyControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
