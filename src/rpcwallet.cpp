...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    if (!pwalletMain->Unlock(strWalletPass, anonymizeOnly))
        throw JSONRPCError(RPC_WALLET_PASSBitMoneyASE_INCORRECT, "Error: The wallet passphrase entered was incorrect.");
...
Value encryptwallet(const Array& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending BitMoneys\n" + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
...
Value settxfee(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in BitMoney/kB rounded to the nearest 0.00000001\n"
            "\nResult\n"
...
Value getwalletinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total BitMoney balance of the wallet\n"
...
Value mintzerocoin(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "mintzerocoin <amount>\n"
            "Usage: Enter an amount of BitMoney to convert to zBIT"
            + HelpRequiringPassphrase());
...
Value spendzerocoin(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 5 || params.size() < 4)
        throw runtime_error(
            ...
            "Overview: Convert zBIT (zerocoins) into BitMoney. \n"
            "amount: amount to spend\n"
            "mintchange: if there is left over BitMoney (change), the wallet can convert it automatically back to zerocoins [true]\n"
