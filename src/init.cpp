...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the BitMoney and zBIT money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 49444, 5520));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in BitMoney/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in BitMoney/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in BitMoney/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-backupzBitMoney=<n>", strprintf(_("Enable automatic wallet backups triggered after each zBIT minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizeBitMoneyamount=<n>     " + strprintf(_("Keep N BitMoney anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
			FormatParagraph(strprintf(_("Copyright (C) 2017-%i The savenode Core Developers"), COPYRIGHT_YEAR)) + "\n" +
			"\n" +
            ...
            try {
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZBitMoneyMinted();
                        RecalculateZBitMoneySpent();
                    }
                    RecalculateBitMoneySupply(1);
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        bool fEnableZBitMoneyBackups = GetBoolArg("-backupzBitMoney", true);
        pwalletMain->setZBitMoneyAutoBackups(fEnableZBitMoneyBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    if (nAnonymizeBitMoneyAmount < 2) nAnonymizeBitMoneyAmount = 2;
    ...
    /* Denominations
       ...
       For example:
       1BitMoney+1000 == (.1BitMoney+100)*10
       10BitMoney+10000 == (1BitMoney+1000)*10
    */
